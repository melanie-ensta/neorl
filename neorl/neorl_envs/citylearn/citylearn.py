...
def auto_size(buildings):
    for building in buildings.values():
        ...
        # print('dhw_storage_capacity {} kW'.format(building.dhw_storage.capacity))
        # print('cooling_storage_capacity {} kW'.format(building.cooling_storage.capacity))
...
def building_loader(data_path, building_attributes, weather_file, solar_profile, building_ids, buildings_states_actions, save_memory = True):
    with open(building_attributes, "r") as json_file:
    ...
    for uid, attributes in zip(data, data.values()):
        if uid in building_ids:
            ...
            simulation_data = os.path.join(data_path, data_file)
            with open(simulation_data, "r") as csv_file:
                data = pd.read_csv(csv_file)
            #data = pd.read_csv(simulation_data)
            ...
            with open(weather_file, "r") as csv_file:
            ...
            with open(solar_profile, "r") as csv_file:
                data = pd.read_csv(csv_file)
...
class CityLearn(core.EnvData):  
    def __init__(self, 
        ...
        if self.simulation_period_len is None:
            ...
            start_index=self.simulation_period_total[0]
            stop_index=self.simulation_period_total[1]
            self.simulation_period = (start_index, stop_index)
        self.uid = None
        self.n_buildings = len([i for i in self.buildings])
        self.reset()

    def set_custom_reward_function(self, reward_func):
        self.custom_reward_function = reward_func
    ...
    def step(self, actions):
        ...
        if self.central_agent:
            ...
            for uid, building in self.buildings.items():
                ...
                building_electric_demand = np.round(_electric_demand_cooling + _electric_demand_dhw + _non_shiftable_load - _solar_generation, 4)
        ...
        if self.central_agent:
            ...
            excess=elec_generation-(elec_consumption_cooling_total-elec_consumption_cooling_storage)-(elec_consumption_dhw_total-elec_consumption_dhw_storage)-elec_consumption_appliances
            if self.custom_reward_function is None:
                rewards = reward_function_sa(self.buildings_net_electricity_demand)
            else:
                rewards = self.custom_reward_function(self.buildings_net_electricity_demand,self.state[2],action_help,excess)
    ...
    def reset(self):
        ...
        simulation_period_len = self.simulation_period_len
    ...
    def cost(self):
        ...
        if self.cost_rbc is None:
            env_rbc = CityLearn(self.data_path, 
                                ...
                                simulation_period_len=self.simulation_period_total[1]-self.simulation_period_total[0], 
                                simulation_period_total = (0,8759), 
            ...
            while not done:
                ...
                next_state, rewards, done, _ = env_rbc.step(np.array(action[0]))
                state = next_state
